...
# The .pkl is a dict saved by src/train_svm.py
bundle = joblib.load(MODEL_PATH)
svm = bundle["model"]
scaler = bundle["scaler"]
label_encoder = bundle["label_encoder"]
CLASSES = list(label_encoder.classes_)  # ['Early_blight', 'Healthy', 'Late_blight', 'Leaf_mold']

# VGG16 feature extractor (ONNX) — same features the SVM was trained on
...
def extract_features(image_bytes: bytes) -> np.ndarray:
    ...
    arr = np.asarray(img, dtype=np.float32)[None, ...]  # (1, 224, 224, 3), RGB
    arr = arr[..., ::-1] - VGG_BGR_MEAN                 # -> BGR, mean-subtracted
    return session.run(None, {INPUT_NAME: arr})[0]      # (1, 512)
...
async def predict(file: UploadFile = File(...)):
    try:
        ...
        features = scaler.transform(features)  # training used StandardScaler

        pred_idx = int(svm.predict(features)[0])          # encoded label (0-3)
        proba = svm.predict_proba(features)[0]
        disease = label_encoder.inverse_transform([pred_idx])[0]

        # Wrapped in {success, data} — the shape the Node backend expects.
        return {
            "success": True,
            "data": {
                "disease": disease,
