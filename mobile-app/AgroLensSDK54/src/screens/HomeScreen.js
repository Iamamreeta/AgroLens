...
import { predictDisease, updateUserStats, getCurrentUser, getHistory } from '../services/AuthService';
import { testConnection } from '../config/api';

function useRefreshByKey(key) {
  const [stamp, setStamp] = useState(0);
  return {
    refresh: useCallback(() => setStamp((s) => s + 1), [key]),
    stamp,
  };
}
...
export default function HomeScreen({ navigation }) {
  ...
  const authRefresh = useRefreshByKey('auth');
  ...
  const loadRecentScan = useCallback(async () => {
    try {
      const r = await getHistory({ limit: 1, offset: 0 });
      if (r && r.history && r.history.length > 0) {
        setRecentScan(r.history[0]);
        return;
      }
      const saved = await AsyncStorage.getItem('predictions');
      if (saved) {
        const arr = JSON.parse(saved);
        if (arr && arr.length > 0) setRecentScan(arr[0]);
      }
    } catch (_) {}
  }, []);
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        if (status !== 'granted') {
          Alert.alert('Permission needed', 'Camera permission is required to capture tomato leaf photos.');
        }
      } catch (_) {}
      testConnection()
        .then((r) => setConnectionStatus(r))
        .catch(() => setConnectionStatus({ ok: false }));
      loadData();
    })();
  }, [loadData, authRefresh.stamp]);
  ...
  const detectDisease = async () => {
    ...
    try {
      ...
      const imageName = image.fileName || `photo-${Date.now()}.jpg`;
      const res = await predictDisease({ uri: imageUri, type: imageType, name: imageName });
      if (!res.success) {
        Alert.alert('Prediction Error', res.error || 'Unable to run prediction. Please try again.');
        return;
      }
      ...
      const fullPred = {
        ...
        image_url_local: imageUri,
      };
      setResult(fullPred);
      setRecentScan(fullPred);
      await updateUserStats(fullPred);
          ...
          <View>
            ...
            {connectionStatus ? (
              <View style={{ flexDirection: 'row', alignItems: 'center', marginTop: 4 }}>
                <Ionicons
                  name={connectionStatus.ok ? 'cloud-done' : 'cloud-offline'}
                  size={14}
                  color={connectionStatus.ok ? '#2e7d32' : '#d32f2f'}
                  style={{ marginRight: 4 }}
                />
                <Text style={{ fontSize: 12, color: connectionStatus.ok ? '#2e7d32' : '#d32f2f' }}>
                  {connectionStatus.ok ? 'Connected to backend' : 'Offline — cached data available'}
                </Text>
              </View>
            ) : null}
          </View>
          ...
          <Animated.View style={[styles.resultCard, { opacity: fadeAnim }]}>
            ...
            <View style={styles.resultBody}>
              {!result.is_leaf ? (
                ...
                <>
                  <View style={styles.resultRow}>
                    ...
                    <Text style={styles.resultDisease}>{result.disease_name || result.disease}</Text>
                  </View>
                  <View style={styles.resultRow}>
                      ...
                      <View style={styles.confidenceBar}>
                        <View
                          style={[
                            ...
                            {
                              width: `${Math.min(100, Math.max(0, Number(result.confidence) || 0))}%`,
                              backgroundColor:
                                Number(result.confidence) > 70 ? '#2ecc71' : '#f39c12',
                            },
                          ]}
                        />
                      </View>
              ...
              <View style={styles.resultActions}>
                ...
                <TouchableOpacity style={styles.closeResultBtn} onPress={() => setResult(null)}>
        ...
        {recentScan && (
          <View style={styles.recentSection}>
            ...
            <TouchableOpacity
              ...
              onPress={() => navigation.navigate('Results', { result: recentScan })}
            >
              <View
                style={[
                  styles.statusBadge,
                  recentScan.status === 'Healthy' ? styles.healthyBadge : styles.diseasedBadge,
                ]}
              >
              ...
              <View style={styles.recentInfo}>
                <Text style={styles.recentDisease}>
                  {recentScan.disease_name || recentScan.disease}
                </Text>
                <Text style={styles.recentDate}>
                  {new Date(recentScan.timestamp || recentScan.created_at || Date.now()).toLocaleString()}
                </Text>
              </View>
...
const styles = StyleSheet.create({
  ...
  resultDisease: { fontSize: 17, fontWeight: '700', color: '#1a3a2a', flex: 1, textAlign: 'right' },
